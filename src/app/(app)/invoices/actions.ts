...
export async function sendInvoiceViaLine(
  ...
  if (!built) return { error: "ไม่พบบิล" };
  if (!built.lineUserId) return { error: "ผู้เช่ายังไม่ได้เชื่อมบัญชี LINE" };

  const res = await pushMessage(built.lineUserId, [textMessage(built.text)]);
